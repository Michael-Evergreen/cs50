...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in possible_actions:
            if not (state, action) in self.q:
                ...
                list_of_values.append(self.q[state, action])
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in possible_actions:
            ...
            temp_dict[(state, action)] = self.get_q_value(state,action)
        if epsilon is False:
            a = max(temp_dict.items(), key=operator.itemgetter(1))[0]
            return a[1]
        else:
            chance = self.epsilon * 100
            dice = random.randint(1, 100)
            if dice <= chance:

                return random.choice(list(temp_dict.keys()))[1]
            else:
                a = max(temp_dict.items(), key=operator.itemgetter(1))[0]
                return a[1]
